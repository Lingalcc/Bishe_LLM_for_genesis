...
from pipeline.app.app_common import extract_first_json_from_text
...
class LocalLLMEngine:
    """Local text-generation engine for robot-action JSON generation.

    Backend selection policy:
    1) Prefer `vllm` when available.
    2) Fallback to `transformers` when `vllm` is unavailable or init fails.
    """
    ...
    def generate(self, prompt: str, temperature: float = 0.0) -> str:
        """Run generation and return a normalized JSON string.

        Args:
            prompt: Natural language instruction.
            temperature: Sampling temperature. Use 0.0 for deterministic decoding.

        Returns:
            JSON string generated by the model.

        Raises:
            RuntimeError: Backend is not initialized or generation fails.
            ValueError: Prompt is empty, temperature is invalid, or no JSON is found.
        """
        ...
        if self.backend == "vllm":
            raw_text = self._generate_with_vllm(prompt=prompt, temperature=temperature)
        elif self.backend == "transformers":
            raw_text = self._generate_with_transformers(prompt=prompt, temperature=temperature)
        else:
            raise RuntimeError("No backend is initialized. Cannot generate text.")

        payload = extract_first_json_from_text(raw_text)
    ...
    def _initialize_backend(self) -> None:
        ...
        try:
            ...
            init_errors.append(f"vllm init failed: {type(exc).__name__}: {exc}")
        ...
        try:
            ...
            init_errors.append(f"transformers init failed: {type(exc).__name__}: {exc}")

        err_msg = "Failed to initialize any local inference backend.\n" + "\n".join(init_errors)
    ...
    def _generate_with_vllm(self, prompt: str, temperature: float) -> str:
    ...
    def _default_max_new_tokens(self) -> int:
        return max(128, min(1024, self.max_model_len // 4))
