...
def main() -> None:
    ...
    section = get_section(cfg, "dataset_prepare", "calibration")

    dataset_file = args.dataset_file or Path(
        str(section.get("dataset_file", "data_prepare/genesis_franka_toolcall_alpaca.json"))
    )
    max_print_errors = args.max_print_errors
    if max_print_errors is None:
        max_print_errors = int(section.get("max_print_errors", 10))
    strict = args.strict if args.strict is not None else bool(section.get("strict", False))
