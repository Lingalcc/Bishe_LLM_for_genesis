...
OUTPUT_ROOT = REPO_ROOT / "output/exp1_data_scale_lora_continue"

TEMP_DIR = EXPERIMENT_DIR / ".cache/lora_continue"
...
def main(force_resplit: bool = False) -> None:
    ...
    bootstrap_eval_config_path = _prepare_eval_config(
        _build_model_output_dir(DATA_SIZES[0]),
        _build_eval_report_path(DATA_SIZES[0]),
    )
    ...
    print(f"[info] Running continuation scales: {effective_sizes}", flush=True)
    if skipped_sizes:
        print(f"[warn] Skipped data scales beyond train size: {skipped_sizes}", flush=True)
    print(f"[info] Finetune method: {FINETUNE_METHOD}", flush=True)
    ...
    print(f"[info] Detail reports dir: {DETAIL_REPORTS_DIR}", flush=True)
    ...
    if completed_sizes:
        print(f"[resume] Existing completed sizes: {sorted(completed_sizes)}", flush=True)
    ...
    if not remaining_sizes:
        print("[resume] All continuation scales are already completed.", flush=True)
        if results:
    ...
    try:
        for data_size in remaining_sizes:
            ...
            metrics = _extract_metrics(eval_report_path)
            row = {
                "data_size": data_size,
        ...
        print(f"\n[restore] Restored continuation train split snapshot: {effective_train_file}", flush=True)
